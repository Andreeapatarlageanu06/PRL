...
std::set<unsigned int> verticesWithInRange(vec3 pos, float distance, skinning_structure& skinning) {
    std::set<unsigned int> closePoints;
    for(int k = 0; k < skinning.deformed.position.size(); k++){
        if(norm(skinning.deformed.position[k]-pos) < distance) {
            closePoints.insert(k);
        }
    }
...
buffer<float> diffuse_laplacien_weights(buffer<float> const& weights,
                                        ...
                                        float lambda, int number_of_steps, skinning_structure& skinning)
{
    ...
    for(int k_step=0; k_step<number_of_steps; ++k_step)
    {
        ...
        {
            auto neigh_set = verticesWithInRange(skinning.deformed.position[k], 0.1f, skinning);
            size_t const N_neigh = neigh_set.size();
