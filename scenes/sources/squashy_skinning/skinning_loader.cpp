...
void load_rondinella_data(skeleton_structure& skeleton, skinning_structure& skinning, vcl::buffer<float>& weight_flappy, vcl::buffer<float>& weight_squashy,vcl::mesh_drawable& shape_visual, vcl::timer_interval& timer, GLuint shader)
{
    ...
    // Cylinder shape
    mesh sphere;
    //sphere = mesh_load_file_obj("assets/rondinella/rondinellaUV_upside_down.obj");
    sphere = mesh_load_file_obj("assets/cactus/cactus.obj");

       for(int k=0; k<int(sphere.position.size()); ++k)
        sphere.position[k] *= 4.0f;
...
data_loaded load_long_cylinder_data(GLuint shader)
{
    data_loaded data = load_basic_data_directory( {"assets/cylinder/", shader, 0.11f} );
...
data_loaded load_dragon_data(GLuint shader)
{
    ...
    data_loaded data = load_basic_animation_data_directory( {"assets/dragon/", shader, 0.09f} );
    return data;
}
...
{
    data_loaded data = load_basic_animation_data_directory( {"assets/custom/", shader, 0.8f} );
    return data;
}

data_loaded load_rayfish_data(GLuint shader)
{
    data_loaded data = load_basic_animation_data_directory( {"assets/RayFish/", shader, 0.8f} );
    return data;
}

data_loaded load_whale_data(GLuint shader)
{
    data_loaded data = load_basic_animation_data_directory( {"assets/Whale/", shader, 0.8f} );
    return data;
}

data_loaded load_hammerhead_data(GLuint shader)
{
    data_loaded data = load_basic_animation_data_directory( {"assets/hammerhead/", shader, 0.8f} );
    return data;
}


data_loaded load_train_data(GLuint shader)
{

    mesh tree;
    buffer<buffer<int> > vertex_correspondance;
    tree = mesh_load_file_obj("assets/cactus/cactus.obj", vertex_correspondance);
    tree.fill_empty_fields();

    data_loaded data = load_basic_animation_data_directory( {"assets/train/", shader, 0.2f} );
...
data_loaded load_clown_data(GLuint shader)
{
    data_loaded data = load_basic_animation_data_directory( {"assets/clown/", shader, 0.2f} );
    return data;
}
data_loaded load_clown_skeleton_hat_data(GLuint shader)
{
    data_loaded data = load_basic_animation_data_directory({ "assets/ClownSkeletonHat/", shader, 0.2f });
    return data;
}
