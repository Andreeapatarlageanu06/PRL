...
void setup_scene(scene_structure &scene, gui_structure& gui, const std::map<std::string,GLuint>& shaders)
{
    ...
    const image_raw pillar_color{1,1,image_color_type::rgba,{196,181,140,255}};
    const image_raw rails_color{1,1,image_color_type::rgba,{128,129,125,255}};
    const image_raw sand{1,1,image_color_type::rgba,{194,178,128,255}};
    const image_raw bars{1,1,image_color_type::rgba,{229,228,223,255}};
    const image_raw green{1,1,image_color_type::rgba,{119,122,68,255}};
    scene.texture_green = create_texture_gpu(green);
    scene.texture_sand = create_texture_gpu(sand);
    scene.texture_pillar = create_texture_gpu(pillar_color);
    scene.texture_rails = create_texture_gpu(rails_color);
    scene.texture_white = create_texture_gpu(white);
    scene.texture_bars = create_texture_gpu(bars);
}
